...
export function generateDoubleEliminationBracket(players: Player[]): {
  ...
  while (currentWinners.length > 1 || currentLosers.length > 0) {
    ...
    const newWinners = winnerMatches
      .map((m) => {
...
export function getNextRoundPlayers(matches: Match[]): Player[] {
  return matches.map((m) => m.winner).filter((p): p is Player => p !== null);
}
...
export function findChampion(tournament: Tournament): Player | null {
  ...
  const winnerBracketWinner = tournament.matches.find(
    (m) =>
      m.round === Math.max(...tournament.matches.map((m) => m.round)) && m.id.startsWith('winner')
  )?.winner;

  const loserBracketWinner = tournament.matches.find(
    (m) =>
      m.round === Math.max(...tournament.matches.map((m) => m.round)) && m.id.startsWith('loser')
  )?.winner;
